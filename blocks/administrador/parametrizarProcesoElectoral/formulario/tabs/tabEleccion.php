	...
	<tbody> 
                    ...
                    if($resultadoCandidatos)
                    {
                        for($can = 0; $can < count($resultadoCandidatos); $can++)
                        {
                            ...
                            <tr>
                                ...
                                <td>
                                    <img src='<?php echo $rutaCandidatos.$resultadoCandidatos[$can][5]?>' width="150px">
                                </td>
                            </tr>        
		...
		<tr id="fila-base<?php echo $idEleccion?>" class="fila-base">
			...
			<td>
                            ...
                            $atributos["estilo"]="jqueryui";
                            $atributos["anchoEtiqueta"] = 250;
                            $atributos["validar"]="required, minSize[3], custom[integer]";
			...
			<td>
                            <input type='file' name='foto<?php echo $idEleccion;?>[]' id='foto<?php echo $idEleccion;?>[]' class="validate[required]">
                        </td>			
...
if($resultadoEleccion)
    {
        ...
        $variableResumen.= "&opcion=resumenEleccion";
        $variableResumen.= "&usuario=" . $miSesion->getSesionUsuarioId();
        $variableResumen.= "&eleccion=" .$resultadoEleccion[0]['ideleccion'];
        $variableResumen = $this->miConfigurador->fabricaConexiones->crypto->codificar_url($variableResumen, $directorioEnlace);
        
        echo "<a href='".$variableResumen."'>                        
                <img src='".$rutaBloque."/images/acroread.png' width='25px'> <br> Resumen de la Elecci&oacute;n
                </a>"; 
    }
?>        
        <?php
